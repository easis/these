...
export function MediaTile({ media, size, activeList, onOpen, onStatus }: MediaTileProps) {
  ...
  return (
    <article className={`media-tile state-${media.status ?? "none"}`} style={{ minWidth: 0 }} onKeyDown={shortcut}>
      ...
      <div className="media-caption">
        ...
        <span className="tile-actions" aria-label={`Classification for ${media.name}`}>
          <button type="button" disabled={!activeList} className={media.status === "selected" ? "is-selected" : ""} onClick={() => onStatus("selected")} title="Selected (1)" aria-label="Mark selected"><Check size={12} /></button>
          <button type="button" disabled={!activeList} className={media.status === "maybe" ? "is-maybe" : ""} onClick={() => onStatus("maybe")} title="Maybe (2)" aria-label="Mark maybe"><CircleHelp size={12} /></button>
          {media.status ? <button type="button" onClick={() => onStatus(null)} title="Remove (0)" aria-label="Remove from active list"><X size={12} /></button> : null}
        </span>
      </div>
    </article>
  );
