...
export function MediaTile({ media, size, activeList, classificationPending = false, className, onOpen, onStatus }: MediaTileProps) {
  ...
  return (
    <article className={cx(styles.mediaTile, media.status === "selected" && styles.selected, media.status === "maybe" && styles.maybe, className)} style={{ minWidth: 0 }} onKeyDown={shortcut} aria-busy={classificationPending}>
