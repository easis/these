...
import { adjacentZoomLevel, clampImagePan, imageFitScale, type Point2D, type Size2D } from "../lib/image-zoom";
...
export function ImageZoom({ path, name, src, layoutKey }: ImageZoomProps) {
  ...
  const setZoom = useCallback((nextScale: number) => {
    ...
    setPan((current) => clampImagePan(current, imageSize, viewportSize, limited));
  ...
  const step = useCallback((direction: 1 | -1) => setZoom(adjacentZoomLevel(scale, direction)), [scale, setZoom]);
  ...
  return <div ref={viewportRef} className={`${styles.zoomViewport} ${pannable ? styles.zoomPannable : ""}`} onPointerDown={startPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan}>
    ...
    />
    <div className={styles.zoomControls} role="group" aria-label="Image zoom controls">
