...
export function ListsPage() {
  ...
  return (
    <div className={content.pageScroll}><div className={content.contentPage}>
      <div className={content.pageTitleRow}><div><p className={content.eyebrow}>Selections</p><h1>Lists</h1><p>Each list keeps one state per file: Selected or Maybe.</p></div><button className={cx(ui.compactButton, ui.primary)} type="button" onClick={() => setCreating(true)}><Plus size={14} />New list</button></div>
      ...
      {bootstrap?.lists.length ? <div className={styles.listIndex}>
        {bootstrap.lists.map((list) => (
          <article key={list.id} className={cx(styles.listIndexRow, activeList?.id === list.id && styles.active)}>
            ...
            <Link to={`/lists/${list.id}`} className="min-w-0 flex-1"><strong className="truncate">{list.name}</strong><span className={styles.listCounts}><span><Check size={13} />{list.selectedCount} selected</span><span><CircleHelp size={13} />{list.maybeCount} maybe</span></span></Link>
...
function ListEmptyMark() { return <div className={styles.emptyMark}><Check size={17} /><CircleHelp size={17} /></div>; }
