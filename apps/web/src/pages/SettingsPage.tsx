    ...
    <div className={content.pageScroll}><div className={cx(content.contentPage, content.narrow)}>
      ...
      <section className={styles.settingsSection}>
        ...
        <form className={styles.rootForm} onSubmit={(event) => void saveRoot(event)}>
          ...
          <label className={styles.rootPathField}><span>Path in These</span><input required value={draft.path} onBlur={() => setPathTouched(true)} onChange={(event) => { setDraft((current) => ({ ...current, path: event.target.value })); setPathTouched(true); }} placeholder="/media/photos" aria-label="Path in These" aria-invalid={pathTouched && Boolean(pathError)} aria-describedby={pathTouched && pathError ? "these-path-help these-path-error" : "these-path-help"} /></label>
          <div className={styles.rootPathHelp}><small id="these-path-help">The application-visible path, for example <code>/media/photos</code>.</small>{pathTouched && pathError ? <small id="these-path-error" className={styles.fieldError} role="alert">{pathError}</small> : null}</div>
          <button className={cx(ui.compactButton, ui.primary, styles.rootSubmit)} type="submit" disabled={saving || Boolean(pathError)}>{editingId ? <Pencil size={13} /> : <Plus size={13} />}{saving ? "Saving…" : editingId ? "Save" : "Add root"}</button>
          {editingId ? <button className={cx(ui.iconButton, ui.bordered, styles.rootCancel)} type="button" onClick={() => { setEditingId(null); setDraft(emptyDraft); setPathTouched(false); }} title="Cancel editing" aria-label="Cancel editing"><X size={14} /></button> : null}
        </form>
